...
function initRafm() {
    ...
    interconnect.innerHTML = RAFM_ALERTS.filter(function(a) {
        var cat = a.category || a.type || '';
        return cat.toUpperCase().includes('INTERCONNECT');
    }).map(function(alert) {
    ...
    roaming.innerHTML = RAFM_ALERTS.filter(function(a) {
        var cat = a.category || a.type || '';
        return cat.toUpperCase().includes('ROAMING');
    }).map(function(alert) {
        return buildAlertItemWithRaise(alert);
    }).join('') || '<div style="padding:24px;text-align:center;color:var(--text-muted);">No active alerts</div>';
...
function buildAlertItem(alert) {
    ...
    var meta  = alert.detectedAt || (alert.age != null ? alert.age + ' days ago' : '');
    return '<div class="alert-item" onclick="openModal(\'' + alert.id + '\')">' +
        ...
        '<div class="alert-content">' +
            ...
            '<div class="alert-meta"><span class="badge badge-' + sev + '">' + sev.toUpperCase() + '</span><span>' + meta + '</span></div>' +
        '</div>' +
...
function renderVendorHeatmap() {
    ...
    heatmap.innerHTML = VENDOR_DATA.map(function(v) {
        ...
        var bg       = score < 31 ? 'rgba(253,52,156,0.1)' : score < 51 ? 'rgba(245,158,11,0.1)' : 'rgba(0,192,174,0.08)';
        return '<div onclick="openVendorModal(\'' + v.name + '\',' + score + ',\'' + exposure + '\',\'' + issue + '\')" ' +
            ...
            '<div style="font-family:var(--font-mono);font-size:18px;font-weight:700;color:' + color + ';text-align:center;">' + score + '</div>' +
            ...
            '<div style="font-size:10px;color:var(--text-secondary);text-align:center;font-family:var(--font-mono);">' + exposure + '</div>' +
        '</div>';
    }).join('');
}