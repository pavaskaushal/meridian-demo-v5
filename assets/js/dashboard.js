...
function buildKPICard(kpi, isFav) {
    ...
    return '<div class="kpi-card" onclick="openKPIDetail(\'' + kpi.id + '\')">' +
        ...
        '<div class="kpi-card-footer">' +
            ...
            '</div>' +
            '<span style="font-family:var(--font-mono);font-size:9px;color:var(--text-muted);letter-spacing:0.5px;">' + (kpi.system || '') + '</span>' +
        '</div>' +
...
function toggleARPUView(view) {
    ...
    if (view === 'trend') {
        ...
        if (trendBtn) { trendBtn.style.background = 'rgba(255,255,255,0.08)'; trendBtn.style.color = 'var(--text-primary)'; trendBtn.style.fontWeight = '700'; }
        ...
        if (compBtn)  { compBtn.style.background  = 'rgba(255,255,255,0.08)'; compBtn.style.color = 'var(--text-primary)'; compBtn.style.fontWeight = '700'; }
...
function setCircleMetric(metric) {
    if (typeof window.CIRCLE_MAP_METRIC !== 'undefined') {
        window.CIRCLE_MAP_METRIC = metric;
        if (typeof renderCircleMap === 'function') renderCircleMap();
    }
    ['arpu','churn','rev'].forEach(function(m) {
        var btn = document.getElementById('cm-' + m);
        if (btn) {
            btn.style.background = (m === metric || (m === 'rev' && metric === 'revenue')) ? 'rgba(255,255,255,0.08)' : 'transparent';
            btn.style.color = (m === metric || (m === 'rev' && metric === 'revenue')) ? 'var(--text-primary)' : 'var(--text-muted)';
            btn.style.border = '1px solid ' + ((m === metric || (m === 'rev' && metric === 'revenue')) ? 'rgba(255,255,255,0.30)' : 'var(--border-light)');
        }
    });
}
