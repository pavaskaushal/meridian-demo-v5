...
function setCircleMetric(metric) {
    ...
    ['arpu','churn','rev'].forEach(function(m) {
        ...
        if (btn) {
            btn.style.background = (m === metric || (m === 'rev' && metric === 'revenue')) ? 'var(--kpmg-cyan)' : 'transparent';
            btn.style.color = (m === metric || (m === 'rev' && metric === 'revenue')) ? '#0A0F1E' : 'var(--text-muted)';
            btn.style.border = (m === metric || (m === 'rev' && metric === 'revenue')) ? 'none' : '1px solid var(--border)';
        }
    });
}
